...
model{
  ...
  alpha_medi ~ normal(0,1);
  alpha_peai ~ normal(0,1);
  alpha_other ~ normal(0,1);

  // implement the biological model
  for(i in 1:N){
   F_hat[i] = lambda / (1 + alpha_intra * ModelMatrix[i,1] + alpha_arca * ModelMatrix[i,2] + alpha_plde * ModelMatrix[i,3] + alpha_poca * ModelMatrix[i,4] + alpha_trcy * ModelMatrix[i,5] + alpha_vero * ModelMatrix[i,6]+ alpha_medi * ModelMatrix[i,7] + alpha_peai * ModelMatrix[i,8] + alpha_other * ModelMatrix[i,9]);
